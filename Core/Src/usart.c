...
/* USER CODE BEGIN 0 */
uint8_t usart1_rec_buffer[100],usart2_rec_buffer[100],usart3_rec_buffer[100];
uint8_t RxBuffer1,RxBuffer2,RxBuffer3;
volatile uint16_t usart1_rcv_len=0,usart2_rcv_len=0,usart3_rcv_len=0;
/* USER CODE END 0 */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	{
		usart1_rec_buffer[usart1_rcv_len++]=RxBuffer1;			//持续接收数据
	}
	//UART2串口接收
	if(huart->Instance==USART2)															//判断是否是串口2
	{
		usart2_rec_buffer[usart2_rcv_len++]=RxBuffer2;			//持续接收数据
	}
	//UART3串口接收
	if(huart->Instance==USART3)															//判断是否是串口3
	{
		usart3_rec_buffer[usart3_rcv_len++]=RxBuffer3;			//持续接收数据
	}
